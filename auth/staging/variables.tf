...
variable "lambda_authorizer_ip_whitelist" {
  ...
  type        = list(string)
  default     = ["195.24.157.182", "34.199.5.64", "18.233.26.203", "52.72.95.145"]
}

variable "license_gateway_lambdas_list" {
  type = any
  default = {
...
// LIST TO ACTIVATE LAMBDA AUTHORIZER:
// auth_domain/users/show

variable "auth_gateway_lambdas_list" {
  type = any
  default = {
...
variable "real_time_verification_gateway_lambdas_list" {
  type = any
  default = {
...
locals {
  lambda_list_raw_1           = flatten([for endpoint, spec in var.license_gateway_lambdas_list : [for lambda in spec : lambda]])
  lambda_list_dynamic_1       = distinct(local.lambda_list_raw_1)
  license_lambda_list_filtred = [for i in local.lambda_list_dynamic_1 : i]
}

locals {
  lambda_list_raw_2        = flatten([for endpoint, spec in var.auth_gateway_lambdas_list : [for lambda in spec : lambda]])
  lambda_list_dynamic_2    = distinct(local.lambda_list_raw_2)
  auth_lambda_list_filtred = [for i in local.lambda_list_dynamic_2 : i]
}

locals {
  lambda_list_raw_3                          = flatten([for endpoint, spec in var.real_time_verification_gateway_lambdas_list : [for lambda in spec : lambda]])
  lambda_list_dynamic_3                      = distinct(local.lambda_list_raw_3)
  real_time_verification_lambda_list_filtred = [for i in local.lambda_list_dynamic_3 : i]
}
